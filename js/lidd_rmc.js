$(document).ready(function() {
	...
	detailsDiv = $('#lidd_rmc_details');
	resultDiv = $('#lidd_rmc_results');
	summaryDiv = $('#lidd_rmc_summary');
	
	$( "#lidd_rmc_form" ).submit(function( event ) {
		...
		// Get variables.
		var ta = $('#lidd_rmc_total_amount').val();
		var dp = $('#lidd_rmc_down_payment').val();
		var ir = $('#lidd_rmc_interest_rate').val();
		var am = $('#lidd_rmc_amortization').val();
		var pp = $('#lidd_rmc_payment_period option:selected' ).val();
		
		// Get the error reporting spans.
		var ta_error = $('#lidd_rmc_total_amount-error');
		var dp_error = $('#lidd_rmc_down_payment-error');
		var ir_error = $('#lidd_rmc_interest_rate-error');
		var am_error = $('#lidd_rmc_amortization-error');
		...
		if ( $.isNumeric( +am ) && Math.abs(+am) < 50 && (+am) !== 0 ) {
		...
		if ( error === false ) {
			...
			resultDiv.prepend( '<p>Payments: <b class="lidd-b">$' + result + '</b></p>');
			...
			$('#lidd_rmc_inspector').click(function() {
